(() => {
  ...
  const readConfig = () => {
    const config = readJson(CONFIG_KEY, { endpoint: '', token: '' });
    return {
  ...
  const styles = `
    ...
    #${ROOT_ID} .gs-panel{display:none;width:min(390px,calc(100vw - 24px));margin-bottom:10px;border:1px solid rgba(255,255,255,.85);border-radius:24px;padding:18px;background:rgba(255,255,255,.96);box-shadow:0 25px 70px rgba(71,30,52,.25);backdrop-filter:blur(18px)}
    ...
    #${ROOT_ID} label{display:block;margin-top:11px}
    #${ROOT_ID} label span{display:block;margin-bottom:5px;font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:#64748b}
    #${ROOT_ID} input{width:100%;border:1px solid #e2e8f0;border-radius:14px;padding:11px 12px;font:inherit;font-size:12px;outline:none;background:white}
    #${ROOT_ID} input:focus{box-shadow:0 0 0 3px rgba(5,150,105,.13);border-color:#6ee7b7}
    ...
    #${ROOT_ID} .gs-save{background:#059669;color:white}.gs-sync{background:#1e293b;color:white}.gs-link{background:#f1f5f9;color:#334155}
    #${ROOT_ID} .gs-help{margin-top:12px;padding-top:11px;border-top:1px solid #e2e8f0;font-size:11px;color:#94a3b8}
    .dark #${ROOT_ID}{color:#e2e8f0}.dark #${ROOT_ID} .gs-panel{background:rgba(15,23,42,.97);border-color:#334155}.dark #${ROOT_ID} h3{color:#f8fafc}.dark #${ROOT_ID} p,.dark #${ROOT_ID} label span{color:#94a3b8}.dark #${ROOT_ID} input{background:#0f172a;border-color:#334155;color:#e2e8f0}.dark #${ROOT_ID} .gs-link{background:#334155;color:#e2e8f0}.dark #${ROOT_ID} .gs-help{border-color:#334155}
  ...
  const refreshUi = () => {
    ...
    const endpoint = root.querySelector('#gs-endpoint');
    const token = root.querySelector('#gs-token');
    if (endpoint && document.activeElement !== endpoint) endpoint.value = config.endpoint;
    if (token && document.activeElement !== token) token.value = config.token;
  };
  ...
  const createUi = () => {
    ...
    root.innerHTML = `
      <div class="gs-panel">
        ...
        <p>Envie automaticamente os gastos deste navegador para a aba <strong>Lançamentos</strong>.</p>
        <div class="gs-status">Configure a integração para começar.</div>
        <label><span>URL do Apps Script</span><input id="gs-endpoint" type="url" placeholder="https://script.google.com/macros/s/.../exec"></label>
        <label><span>Código secreto</span><input id="gs-token" type="password" placeholder="Código definido no Apps Script"></label>
        <div class="gs-actions">
          <button class="gs-button gs-save" type="button">Salvar integração</button>
        ...
        <div class="gs-help">A URL e o código ficam somente neste navegador. Configure uma vez em cada aparelho usado por Pablo ou Ana.</div>
      </div>
    ...
    root.querySelector('.gs-save').addEventListener('click', () => {
      const endpoint = String(root.querySelector('#gs-endpoint').value || '').trim().replace(/\?.*$/, '');
      const token = String(root.querySelector('#gs-token').value || '').trim();
      if (!/^https:\/\/script\.google\.com\//.test(endpoint) || !endpoint.endsWith('/exec')) {
        setStatus('Cole a URL de implantação terminada em /exec.', true);
        return;
      }
      if (token.length < 12) {
        setStatus('O código secreto precisa ter pelo menos 12 caracteres.', true);
        return;
      }
      localStorage.setItem(CONFIG_KEY, JSON.stringify({ endpoint, token }));
      setStatus('Integração salva. Enviando lançamentos pendentes...');
      refreshUi();
      syncNow();
    });
  ...
  const sendExpense = async (config, entry) => {
    const payload = {
    ...
    try {
      ...
      console.warn('[Google Planilhas] Envio sem confirmação do navegador:', firstError);
      return false;
    }
  };
  ...
  async function syncNow() {
    ...
    if (!config.endpoint || !config.token) {
      setStatus('Abra a configuração e informe a URL e o código secreto.', true);
      return;
    }
    ...
    try {
      ...
      for (const entry of pending) {
        const confirmed = await sendExpense(config, entry);
        usedFallback ||= !confirmed;
        synced.add(entry.id);
  ...
  const tick = () => {
    ...
    createUi();
    refreshUi();
    if (pendingEntries().length && readConfig().endpoint && readConfig().token) syncNow();
  };
