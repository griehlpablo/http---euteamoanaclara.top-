...
const LEVELS = {
  perfect: { 
    id: 'perfect', 
    label: 'Amorzinho Perfeito', 
    resumo: 'Amorzinho Perfeito! ✨',
    icon: Heart, color: 'bg-rose-500', text: 'text-rose-500', val: 5, 
    frase: "Prepara o coração que o nível de amor explodiu! O dia hoje está maravilhoso ao seu lado." 
  },
  good: { 
    id: 'good', 
    label: 'Muito Bonzinho(a)', 
    resumo: 'Muito Bonzinho(a)! ⭐',
    icon: Star, color: 'bg-amber-400', text: 'text-amber-500', val: 4, 
    frase: "Alguém está ganhando muitos pontos hoje! Continue assim que o prêmio vai ser grande." 
  },
  chill: { 
    id: 'chill', 
    label: 'De Boa / Chill', 
    resumo: 'Clima De Boa / Chill ☕',
    icon: Coffee, color: 'bg-blue-400', text: 'text-blue-500', val: 3, 
    frase: "Clima tranquilo e favorável por aqui. Nem muito frio, nem muito quente, apenas paz." 
  },
  ice: { 
    id: 'ice', 
    label: 'Em Gelo Fino', 
    resumo: 'Cuidado: Em Gelo Fino ❄️',
    icon: Snowflake, color: 'bg-cyan-400', text: 'text-cyan-500', val: 2, 
    frase: "Cuidado! O clima esfriou... melhor agir rápido antes que o gelo quebre de vez!" 
  },
  jail: { 
    id: 'jail', 
    label: 'Cadeia (Aiai)', 
    resumo: '🚨 VOCÊ FOI PRESO(A)!',
    icon: Lock, color: 'bg-slate-800', text: 'text-slate-700', val: 1, 
    frase: "ALERTA: Você foi detido(a) pela polícia do amor. Confira sua missão urgente para ser solto(a)." 
  }
};
    ...
    return (
      <div className="relative h-[160px] w-full pt-4">
        <svg className="w-full h-full overflow-visible" preserveAspectRatio="none">
          <polyline points={points} fill="none" stroke={chartTab === 'pablo' ? '#3b82f6' : '#f43f5e'} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
        </svg>
      </div>
    );
  };
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
          ...
          <div className="flex gap-4 w-full max-w-sm bg-slate-100 p-1.5 rounded-2xl relative z-50 mb-3">
            <button 
              ...
              className={`flex-1 py-3 px-4 rounded-xl font-bold cursor-pointer ${currentUser === 'pablo' ? 'bg-blue-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}
            >
              Pablo
            </button>
            <button 
              onClick={() => setCurrentUser('ana')} 
              className={`flex-1 py-3 px-4 rounded-xl font-bold cursor-pointer ${currentUser === 'ana' ? 'bg-rose-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}
            >
              Ana
            </button>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <button 
              ...
              className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-full cursor-pointer hover:bg-blue-700 transition-colors"
            >
              <Zap size={14} /> Ativar iPhone
            </button>
            <button 
              onClick={() => { if(OneSignal.User) OneSignal.User.addTag('usuario', currentUser); alert("Vínculo OK!"); }} 
              className="px-4 py-2 bg-slate-800 text-white text-xs font-bold rounded-full cursor-pointer hover:bg-slate-700 transition-colors"
            >
        ...
        {currentUser && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4">
              ...
              {Object.values(LEVELS).map((level) => {
                ...
                return (
                  <button 
                    ...
                    className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer ${isSelected ? `${level.color} border-transparent text-white shadow-lg` : 'border-slate-100 bg-white text-slate-500 hover:border-slate-300'}`}
                  >
            ...
            <button 
              ...
              className="w-full mt-8 py-3 bg-slate-100 text-slate-500 text-xs font-bold rounded-xl flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-200 transition-colors"
            >
