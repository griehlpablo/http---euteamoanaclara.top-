const DEFAULT_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwZLBQWBNaztWMrQwPieOI1oRcZb-QgjoskdykE9I1loAMQ5agTXrwD9jzjIT6d07iv/exec';
...
export default {
  async fetch(request) {
    ...
    if (body?.action !== 'appendExpense' || !isValidExpense(body?.expense)) {
    ...
    try {
      ...
      try {
        ...
        return json({ ok: false, error: 'O Apps Script respondeu em formato inválido.' }, 502);
      }

      if (!upstream.ok || !result.ok) {
        return json({
          ok: false,
          error: result.error || `Apps Script respondeu com status ${upstream.status}.`,
        }, 502);
      }
