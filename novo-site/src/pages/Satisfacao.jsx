...
import { ArrowLeft, Heart, Star, Coffee, Snowflake, Lock, TrendingUp, User, PenSquare, Smartphone, Zap } from 'lucide-react';
...
export default function Satisfacao() {
  ...
  const ativarNotificacoesNativas = async () => {
    if (OneSignal) {
      ...
      alert("Se o pop-up cinza apareceu, autorize e clique em 'Confirmar Vínculo'!");
    }
  };
  ...
  const enviarNotificacaoProAmor = async (nivelLabel, mensagemExtra = "") => {
    ...
    const msg = mensagemExtra || `${meuNome} atualizou o nível para: ${nivelLabel}`;
  ...
  const saveJailNote = () => {
    ...
    enviarNotificacaoProAmor("Cadeia", `🚨 Missão para sair da cadeia: "${jailInput}"`);
    setJailInput('');
  };

  const renderChart = () => {
    const targetHistory = history.filter(h => h.target === chartTab).slice(-15); 
    if (targetHistory.length < 2) return <div className="h-48 flex items-center justify-center text-slate-400 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200"><p>Faltam votos.</p></div>;
        ...
        {currentUser && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center mb-6">
              ...
              <p className="text-slate-500 text-sm mt-1">Status: <strong className={currentTargetLevel.text}>{currentTargetLevel.label}</strong></p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 relative z-50">
              {Object.values(LEVELS).map((level) => {
                ...
                return (
                  <button key={level.id} onClick={() => handleVote(level.id)} className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer ${isSelected ? `${level.color} border-transparent text-white shadow-lg` : 'border-slate-100 bg-white text-slate-500'}`}>
