...
const extractJson = (value) => {
  ...
  if (start < 0 || end < start) throw new Error('A resposta não contém dados válidos.');
  return JSON.parse(sanitized.slice(start, end + 1));
};

const normalizeChoice = (value, choices, fallback) => {
  const normalized = String(value || '').trim().toLocaleLowerCase('pt-BR');
  return choices.find((choice) => choice.toLocaleLowerCase('pt-BR') === normalized) || fallback;
};
...
export default function Gastos() {
  ...
  const personTotals = useMemo(
    () =>
      monthEntries.reduce((acc, entry) => {
        acc[entry.person] = (acc[entry.person] || 0) + Number(entry.amount || 0);
        return acc;
      }, {}),
    [monthEntries],
  );
  ...
  const budgetPercentage = Math.min(100, Math.max(0, (monthTotal / MONTHLY_SPENDING_LIMIT) * 100));
  ...
  const interpretExpense = async () => {
    ...
    try {
      ...
      const person = normalizeChoice(parsed.person, ['Pablo', 'Ana'], defaultPerson);
      const defaultAccount = person === 'Pablo' ? 'Banco do Brasil — Pablo' : 'Banco do Brasil — Ana';
      const amount = parseAmount(parsed.amount);

      setForm({
        date: isValidDateKey(parsed.date) ? parsed.date : todayKey(),
        description: String(parsed.description || quickEntry || 'Compra').trim(),
        category: normalizeChoice(parsed.category, CATEGORIES, 'Outros/imprevistos'),
        person,
        account: normalizeChoice(parsed.account, ACCOUNTS, defaultAccount),
        paymentMethod: normalizeChoice(parsed.paymentMethod, PAYMENT_METHODS, 'Pix'),
      ...
      window.setTimeout(() => document.getElementById('gasto-form')?.scrollIntoView({ behavior: 'smooth' }), 50);
    } catch (error) {
      setMessage(`Não consegui interpretar esse gasto: ${error?.message || String(error)}`);
    } finally {
  ...
  const analyzeMonth = async () => {
    if (!monthEntries.length) {
      setAnalysis('Registre alguns gastos deste mês para eu conseguir fazer uma análise útil.');
      return;
    }
    ...
    try {
      const snapshot = {
        ...
        byPerson: Object.fromEntries(
          Object.entries(personTotals).map(([person, total]) => [person, Number(total.toFixed(2))]),
        ),
      ...
      setAnalysis(`Não consegui analisar o mês agora: ${error?.message || String(error)}`);
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(`Registrado e sincronizado na planilha: ${entry.description} por ${currency.format(entry.amount)}.`);
    } catch {
      setMessage(`Registrado neste aparelho. A sincronização com a planilha continuará tentando automaticamente.`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const deleteEntry = (id) => {
    ...
    setMessage('Lançamento removido deste aparelho. A exclusão não altera uma linha já enviada à planilha.');
  };
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto py-6">
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Registro, análise e organização das compras de Pablo e Ana.</p>
        </div>
      </div>
      ...
      <section className="mb-6 rounded-[2rem] border border-violet-100 bg-gradient-to-br from-violet-50/90 via-white/80 to-rose-50/90 p-5 shadow-xl dark:border-violet-900/50 dark:from-slate-800/90 dark:via-slate-900/80 dark:to-violet-950/60">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
          <div className="flex-1">
            <div className="mb-3 flex items-center gap-3">
              <div className="rounded-2xl bg-violet-100 p-3 text-violet-600 dark:bg-violet-950 dark:text-violet-300"><Sparkles /></div>
              <div>
                <h2 className="font-serif text-2xl font-bold text-slate-800 dark:text-slate-100">Assistente financeiro</h2>
                <p className="text-xs text-slate-500 dark:text-slate-400">Escreva do seu jeito ou envie uma foto. O formulário será preenchido para você conferir.</p>
              </div>
            </div>
            ...
            {receiptFile && (
              <div className="mt-2 flex items-center justify-between rounded-2xl bg-white/70 px-4 py-3 text-sm text-slate-600 dark:bg-slate-950/50 dark:text-slate-300">
                ...
                <button type="button" onClick={() => { setReceiptFile(null); if (receiptInputRef.current) receiptInputRef.current.value = ''; }} className="rounded-full p-1 text-slate-400 hover:text-red-500"><X size={17} /></button>
              </div>
            )}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" onClick={interpretExpense} disabled={isInterpreting} className="inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-4 py-3 text-sm font-bold text-white hover:bg-violet-700 disabled:opacity-60">
                {isInterpreting ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
                Preencher lançamento
              </button>
              <button type="button" onClick={() => receiptInputRef.current?.click()} className="inline-flex items-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-sm dark:bg-slate-800 dark:text-slate-200">
                <Camera size={18} /> Ler comprovante
              </button>
              <button type="button" onClick={analyzeMonth} disabled={isAnalyzing} className="inline-flex items-center gap-2 rounded-2xl bg-slate-800 px-4 py-3 text-sm font-bold text-white hover:bg-slate-700 disabled:opacity-60">
          ...
          <div className="w-full rounded-3xl bg-white/75 p-4 text-sm shadow-sm dark:bg-slate-950/50 lg:w-80">
            <p className="text-xs font-bold uppercase tracking-widest text-violet-500">Metas usadas na análise</p>
            <div className="mt-3 space-y-2 text-slate-600 dark:text-slate-300">
              <div className="flex justify-between gap-3"><span>Limite mensal</span><strong>{currency.format(MONTHLY_SPENDING_LIMIT)}</strong></div>
              <div className="flex justify-between gap-3"><span>Meta de economia</span><strong>{currency.format(MONTHLY_SAVINGS_GOAL)}</strong></div>
              <div className="flex justify-between gap-3"><span>Delivery</span><strong>{currency.format(DELIVERY_LIMIT)}</strong></div>
            </div>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
              <div className={`h-full rounded-full ${budgetPercentage >= 90 ? 'bg-red-500' : budgetPercentage >= 70 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${budgetPercentage}%` }} />
            </div>
            <p className="mt-2 text-xs text-slate-500">{budgetPercentage.toFixed(0)}% do limite mensal utilizado.</p>
          </div>
        </div>
        ...
        {(analysis || isAnalyzing) && (
          <div className="mt-4 rounded-3xl border border-violet-100 bg-white/80 p-5 text-sm text-slate-700 dark:border-violet-900/50 dark:bg-slate-950/60 dark:text-slate-200">
            {isAnalyzing ? (
              <div className="flex items-center gap-2 text-violet-500"><Loader2 size={18} className="animate-spin" /> Analisando os lançamentos deste mês...</div>
            ) : (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <div className="rounded-3xl bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/60 dark:border-slate-700 p-5 shadow-lg">
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Restante do limite</p>
          <p className={`mt-2 text-2xl font-bold ${remainingBudget < 0 ? 'text-red-500' : 'text-emerald-600'}`}>{currency.format(remainingBudget)}</p>
        </div>
        <div className="rounded-3xl bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/60 dark:border-slate-700 p-5 shadow-lg">
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Delivery disponível</p>
          <p className={`mt-2 text-2xl font-bold ${deliveryRemaining < 0 ? 'text-red-500' : 'text-slate-800 dark:text-slate-100'}`}>{currency.format(deliveryRemaining)}</p>
        </div>
        <div className="rounded-3xl bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/60 dark:border-slate-700 p-5 shadow-lg">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <form id="gasto-form" onSubmit={handleSubmit} className="rounded-[2rem] bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/60 dark:border-slate-700 p-6 shadow-xl">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Confira os campos antes de salvar. Não informe cartão, senha ou dados bancários sensíveis.</p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block sm:col-span-2">
              ...
              <input value={form.description} onChange={(event) => updateField('description', event.target.value)} placeholder="Ex.: Coca-Cola 1 L retornável" className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200" required />
            </label>
            ...
            <label className="block">
              ...
              <div className="relative">
                ...
                <input inputMode="decimal" value={form.amount} onChange={(event) => updateField('amount', event.target.value)} placeholder="0,00" className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 py-3 pl-12 pr-4 outline-none focus:ring-2 focus:ring-rose-200" required />
              </div>
            </label>

            <label className="block">
              <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Data</span>
              <input type="date" value={form.date} onChange={(event) => updateField('date', event.target.value)} className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200" required />
            </label>

            <label className="block">
              <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Categoria</span>
              <select value={form.category} onChange={(event) => updateField('category', event.target.value)} className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200">
            ...
            <label className="block">
              ...
              <select value={form.person} onChange={(event) => updateField('person', event.target.value)} className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200">
                <option>Pablo</option>
                <option>Ana</option>
              </select>
            </label>
            ...
            <label className="block">
              ...
              <select value={form.account} onChange={(event) => updateField('account', event.target.value)} className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200">
            ...
            <label className="block">
              ...
              <select value={form.paymentMethod} onChange={(event) => updateField('paymentMethod', event.target.value)} className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200">
            ...
            <label className="block sm:col-span-2">
              ...
              <input value={form.notes} onChange={(event) => updateField('notes', event.target.value)} placeholder="Opcional" className="w-full rounded-2xl border border-white/70 bg-white/80 dark:bg-slate-900/60 px-4 py-3 outline-none focus:ring-2 focus:ring-rose-200" />
            </label>
          </div>
          ...
          <button type="submit" disabled={isSaving} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-rose-500 to-pink-500 px-5 py-4 font-bold text-white shadow-lg hover:from-rose-600 hover:to-pink-600 disabled:opacity-60 transition-all">
        ...
        <div className="space-y-5">
          ...
          <p className="px-2 text-xs leading-relaxed text-slate-400">
            Os lançamentos ficam disponíveis neste navegador e são enviados à planilha quando a sincronização está configurada. A interpretação automática apenas preenche o formulário; nada é salvo sem sua confirmação.
          </p>
        </div>
      </div>
    </motion.div>
