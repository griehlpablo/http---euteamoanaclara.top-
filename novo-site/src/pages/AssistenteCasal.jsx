...
const migrateLocalChats = async () => {
  ...
  for (const localChat of [...localChats].reverse()) {
    ...
    const payload = localMessages.map((message) => ({
      ...
      role: message.role,
      text: message.text || "",
      imageUrl: message.imageUrl?.startsWith("data:")
        ? null
...
export default function AssistenteCasal() {
  ...
  const criarNovoChat = async () => {
    ...
    setChats((previous) => [
      data,
      ...previous.filter((chat) => chat.id !== data.id),
    ]);
    setActiveChatId(data.id);
  };
  ...
  const deletarChatAtivo = async () => {
    if (
      !activeChatId ||
      !window.confirm("Quer mesmo apagar essa conversa inteira? 🧹")
    )
      return;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const notificationRequested =
        /\b(?:avisa|avise|avisar|notifica|notifique|notificar|manda|mande|mandar|fala|fale|falar|pede|peça|pedir)\b[\s\S]{0,60}\bpablo\b/i.test(
          prompt,
        );
      if (hasNotificationTag)
  ...
  return (
    <div className="max-w-4xl mx-auto h-[85vh] flex flex-col md:flex-row gap-4 pt-4 px-2 relative z-10">
      <div className="w-full md:w-64 bg-white/60 backdrop-blur-lg border border-white/50 shadow-xl rounded-[2rem] p-4 flex flex-col h-48 md:h-full flex-shrink-0">
        ...
        <Link
          to="/central"
          className="mt-4 text-center text-slate-400 hover:text-rose-500 text-xs font-bold uppercase flex items-center justify-center gap-1"
        >
      ...
      <div className="bg-white/60 backdrop-blur-lg border border-white/50 shadow-xl flex-1 overflow-hidden flex flex-col rounded-[2rem] p-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400">
              {isSupabaseConfigured
                ? "Conversas sincronizadas na nuvem"
                : "Conversas salvas neste navegador"}
            </span>
          </div>
          {activeChatId && (
            <button
              onClick={deletarChatAtivo}
        ...
        {activeChatId ? (
          <>
            <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2 custom-scrollbar font-sans text-slate-800">
              {messages.map((message, index) => (
                ...
                >
                  <div
                    className={`max-w-[85%] p-4 rounded-2xl text-sm ${message.role === "user" ? "bg-rose-500 text-white rounded-br-none shadow-md" : "bg-white border border-slate-100 shadow-sm"}`}
                  >
                    {message.fileType &&
                    !message.fileType.startsWith("image/") ? (
                      message.imageUrl ? (
                        <a
                          ...
                          className="flex items-center gap-2 bg-white/20 p-2 rounded-lg hover:bg-white/30 transition-colors mb-2 text-white text-sm w-max"
                        >
                          <FileText size={16} /> Documento Anexado
                        </a>
                      ) : (
                        <div className="flex items-center gap-2 bg-white/20 p-2 rounded-lg mb-2 text-sm w-max">
                      ...
                      message.imageUrl && (
                        <img
                          ...
                          alt="Anexo"
                          className="w-full max-w-sm rounded-xl mb-2 object-cover"
                        />
                      )
                    )}
                    {message.role === "user" ? (
              ...
              {isLoading && (
                <div className="text-rose-400 text-xs italic animate-pulse px-4">
                  O Cupido está preparando a resposta...
                </div>
              )}
              <div ref={chatEndRef} />
            </div>
            ...
            <div className="flex flex-col gap-2 bg-white/40 p-2 rounded-2xl border border-rose-100">
                ...
                <div className="relative ml-2 mt-2 inline-block">
                  {selectedFile.type.startsWith("image/") ? (
                    <img
                      src={filePreview}
                      alt="Preview"
                      className="w-20 h-20 object-cover rounded-xl border-2 border-rose-200"
                    />
                  ) : (
              ...
              <div className="flex items-center w-full gap-2">
                ...
                <button
                  ...
                  className="text-rose-400 p-2 rounded-full hover:bg-rose-100"
                >
                  <ImageIcon size={24} />
                </button>
                <input
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-rose-300 font-serif">
            ...
            <p>Crie uma conversa para começar.</p>
          </div>
        )}
      </div>
    </div>
