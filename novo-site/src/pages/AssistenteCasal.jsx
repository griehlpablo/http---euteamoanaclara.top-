...
const callGeminiAPI = async (prompt) => {
  ...
  // URL ATUALIZADA PARA v1 PARA EVITAR ERRO 404
  const url = `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
  
  const systemInstruction = "Você é o 'Cupido Virtual ✨', o assistente romântico do Pablo e da Ana Clara. Pablo criou este site para ela. Seja fofo e use emojis. História: primeiro beijo em 06/07/2023, namoro em 23/09/2023.";
  ...
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "Opa, deu erro aqui. Tente de novo! 💖";
  } catch (error) {
...
export default function AssistenteCasal() {
  ...
  return (
    <div className="max-w-2xl mx-auto h-[70vh] flex flex-col pt-4">
      ...
      <div className="bg-white/60 backdrop-blur-lg border border-white/50 shadow-xl flex-1 overflow-hidden flex flex-col rounded-[2rem] p-4">
        <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2">
          ...
          {isLoading && <div className="text-rose-400 text-xs italic animate-pulse px-4">Digitando...</div>}
          <div ref={chatEndRef} />
        </div>
